...
| **Migration stack** | BAR-Devtools | `just bar::migrate::stylua-cleanup-generate` deterministically rebuilds `fmt` (stylua) → one leaf PR per transform → `mig` (all transforms) → `fmt-llm` (env layer + LLM type-triage to 0). |
...
1. **Reproducibility.** The migration stack — formatting, the type bifurcation,
   the path to 0 errors — is only reproducible via `just bar::migrate::stylua-cleanup-generate`.
