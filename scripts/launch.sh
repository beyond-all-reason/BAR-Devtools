...
#
# WSL2: ensure BAR_DEVSYNC_DIR is set, start the Windows-side sync watcher,
# invoke the bar-launch.cmd shim (Windows-side Python launches the engine
# as a native Windows process), then stop the watcher on exit. The shim is
# generated by `just setup::init` and lives at $BAR_DEVSYNC_DIR/bin/.
#
...
run_wsl() {
  ...
  # Ensure the sync watcher is up. Idempotent. Deliberately NOT trapped on
  # EXIT: the original design stopped it on every launch.sh exit, which
  # compounded with the Electron launcher's ~10-30s graceful shutdown to
  # produce a long perceived "hang" after the game closed. The watcher is
  # cheap (~3MB resident); leave it running. Tear down explicitly via
  # `just bar::stop` when you actually want it gone.
  if ! bash "$DEVTOOLS_DIR/scripts/sync.sh" status >/dev/null 2>&1; then
    info "Starting sync watcher"
    bash "$DEVTOOLS_DIR/scripts/sync.sh" start \
      || { err "sync watcher failed to start"; exit 1; }
  else
    info "sync watcher already running"
  fi
  ...
  printf '\033[0;34m[info]\033[0m         just bar::sync-logs -- -F (sync watcher)\n'
  printf '\033[0;34m[info]\033[0m  stop:  just bar::stop            (sync watcher)\n'
  ...
  ( cd /mnt/c && nohup cmd.exe /c "$shim_win" "$@" </dev/null >/dev/null 2>&1 & )
  return 0
}
