...
# scratch dir for run artifacts; .git is a file when BAR is a linked worktree
BAR_GIT_DIR="$(git -C "$BAR" rev-parse --absolute-git-dir)"
...
# enter_distrobox re-execs via `bash -s < "$0"` — the script itself streams
# over stdin, and any child that reads stdin eats the rest of it, ending the
# run silently at the next top-level command. Re-exec from the file (visible
# in-container; it lives under $HOME) so bash reads a seekable fd instead,
# and park stdin on /dev/null for the whole run.
_SELF="${DEVTOOLS_DIR}/scripts/codemod/generate-branches.sh"
if [[ -n "${_DEVTOOLS_IN_DISTROBOX:-}" ]] && [[ "$0" != "$_SELF" ]]; then
...
# Listed in the order they should land upstream.
#
# stylua-crlf (PR 8549: CRLF + 120-column .stylua.toml, comment sites reshaped
# around StyLua#1140) replaced fix_stylua when it merged. engine-builders-env
# (Spring*->Engine* builders, recovered from mig-spring-split) waits for
# spring-split proper; sharing-modules now owns the modernized builders
# Spring-named. gh-migrate-action-env (the /migrate action) lands on its own
# schedule, not as a prefix.
PREFIX_BRANCHES=("stylua-crlf")
...
TEST_RESULTS_CACHE="$BAR_GIT_DIR/test-results.cache"
...
build_leaf() {
    ...
    local output_file="$BAR_GIT_DIR/${branch}-output.txt"
...
build_mig() {
    ...
    local mig_output_file="$BAR_GIT_DIR/mig-output.txt"
    : > "$mig_output_file"
    for transform in "${TRANSFORMS[@]}"; do
        local commit_msg
        commit_msg=$(tvar "$transform" "commit")
    ...
    local blame_cache="$BAR_GIT_DIR/mig-blame-hashes.txt"
    : > "$blame_cache"
...
build_fmt_llm() {
    ...
    if [[ "$before" == "0" ]]; then
        ...
        # </dev/null: enter_distrobox streams this script over stdin
        # (bash -s < "$0"); a worker that reads stdin would eat the rest of
        # the script and end the run at the next top-level command.
        bash "${DEVTOOLS_DIR}/scripts/codemod/llm-type-triage.sh" </dev/null
    fi
...
generate_all_pr_bodies() {
    ...
    if [[ "${DO_LLM_ONLY:-false}" != "true" ]]; then
        for transform in "${TRANSFORMS[@]}"; do
            ...
            branch=$(tvar "$transform" "branch")
            output_file="$BAR_GIT_DIR/${branch}-output.txt"
            pr_body_file="$BAR_GIT_DIR/${branch}-pr-body.md"
        ...
        local mig_output_file="$BAR_GIT_DIR/mig-output.txt"
        pr_body_file="$BAR_GIT_DIR/mig-pr-body.md"
    ...
    if git_bar rev-parse --verify "$LLM_SOURCE_BRANCH" >/dev/null 2>&1; then
        pr_body_file="$BAR_GIT_DIR/${LLM_SOURCE_BRANCH}-pr-body.md"
    ...
    if git_bar rev-parse --verify "$LLM_BRANCH" >/dev/null 2>&1; then
        pr_body_file="$BAR_GIT_DIR/${LLM_BRANCH}-pr-body.md"
        ...
        local tracking_body_file="$BAR_GIT_DIR/tracking-issue-body.md"
...
generate_llm_pr_body() {
    local summary_file
    summary_file="$BAR_GIT_DIR/llm-triage-summary.txt"
...
update_prs() {
    if [[ "${DO_LLM_ONLY:-false}" != "true" ]]; then
        ...
        if [[ -n "$MIG_PR" ]]; then
            ...
            edit_pr "$MIG_PR" "$BAR_GIT_DIR/mig-pr-body.md" "${PR_BASE[mig]:-master}"
            ok "mig PR updated"
        fi
    fi
...
update_capstone_pr() {
    ...
    local pr_body_file="$BAR_GIT_DIR/${branch}-pr-body.md"
...
push_branches() {
    ...
    # A branch pushed at the same tip as its PR base is read by GitHub as
    # "merged": it closes the PR, credits the pusher, and DELETES the branch.
    # That is irreversible — a merged PR cannot be reopened. It happens when a
    # run dies between `checkout -B <leaf> fmt` and the transform commit, and
    # the empty leaf is pushed anyway (this is how #8403 was lost).
    local -a empty=()
    for b in "${existing[@]}"; do
        local base="${PR_BASE[$b]:-master}"
        git_bar rev-parse --verify "$base" >/dev/null 2>&1 || continue
        if [[ "$(git_bar rev-list --count "$base..$b")" == "0" ]]; then
            empty+=("$b (base $base)")
        fi
    done
    if ((${#empty[@]})); then
        err "Refusing to push: these branches carry no commits over their PR base,"
        err "so GitHub would false-merge and delete them:"
        printf '  %s\n' "${empty[@]}" >&2
        err ""
        err "The generation for these branches did not complete. Re-run it, or drop"
        err "them from the push set — do not push them as-is."
        exit 1
    fi
