...
- **Container recreate** — `cmd_setup_distrobox` always rebuilds: `stop --yes; rm --yes; create`. No "if exists, ask the user; else build".
...
3. Are we maintaining a fallback for an impossible state? (E.g. "what if watchman isn't installed even though dev.Containerfile installs it?" — that's a stale-container problem, fixed by always rebuilding, not by adding a fallback.)
